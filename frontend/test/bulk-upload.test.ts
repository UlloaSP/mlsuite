...
describe("bulk upload helpers", () => {
	...
	it("skips rows with empty names or invalid typed values", () => {
		const emptyName = parseCsvPredictionFile("name,age,active,segment,tags,points\n,1,true,A,,[]", bulkSchema);
		const badNumber = parseCsvPredictionFile("name,age,active,segment,tags,points\nrow,nope,true,A,,[]", bulkSchema);
		const badBoolean = parseCsvPredictionFile("name,age,active,segment,tags,points\nrow,1,maybe,A,,[]", bulkSchema);
		const badSeries = parseCsvPredictionFile("name,age,active,segment,tags,points\nrow,1,true,A,,{}", bulkSchema);
	...
	it("handles quoted commas, malformed quotes, and max overflow", () => {
		const quoted = parseCsvPredictionFile('name,age,active,segment,tags,points\nrow,1,true,"A, B",,"[]"', bulkSchema);
		const malformed = parseCsvPredictionFile('name,age,active,segment,tags,points\nrow,1,true,"A,,[]', bulkSchema);
		const overflow = parseCsvPredictionFile(
			Array.from({ length: 10001 }, (_, index) => `row-${index},1,true,A,,[]`).join(
				"\n",
