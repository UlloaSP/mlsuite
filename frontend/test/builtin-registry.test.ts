...
describe("MLForm builtin registry", () => {
	...
	it("rejects unknown field kinds without treating them as builtins", () => {
		...
		expect(result.issues.some((issue) => issue.message.includes("active plugin catalog"))).toBe(true);
		expect(schemaNeedsActivePluginCatalog({ fields: [{ kind: "missing-custom-field" }] })).toBe(true);
	});
});
