			...
			<div className="flex flex-wrap items-end justify-between gap-4">
				<div>
					...
					<p className="mt-1 text-sm text-[var(--text-secondary)]">
						{filtered.length} of {logLines.length} lines &middot; multi-service tail
						with live filtering.
					</p>
				</div>
					...
					{filtered.length > 0 ? (
						filtered.map((l) => (
							<div key={l.id} className="whitespace-pre-wrap break-words">
								<span className={cx("mr-2 inline-block rounded px-1 py-px text-[0.6rem] font-semibold", levelClass(l.level))}>
									{l.level}
								</span>
								{l.text}
							</div>
