...
import {
  ...
  MemoryBar,
  SegmentedControl,
  ServiceHealthSegment,
  alertTone,
} from "./OverviewViewSupport";
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_320px]">
        <div className="overflow-hidden rounded-xl border border-[var(--border-soft)] bg-[var(--surface-primary)]">
          ...
          <div className="flex items-center justify-between border-t border-[var(--border-soft)] px-5 py-2.5 text-xs text-[var(--text-secondary)]">
            <span>
              {Object.values(layers).filter(Boolean).length} layers &middot; {points.length} samples
            </span>
...
function ServiceMemory({
  ...
  services: InfrastructureOverviewDto["services"];
  totalMem: number;
}) {
  return (
    <div className="overflow-hidden rounded-xl border border-[var(--border-soft)] bg-[var(--surface-primary)]">
