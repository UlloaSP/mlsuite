...
import { clearActiveOrganizationSlug } from "./tenant";
...
const tenantHeader = (): string | null => {
  const value = window.localStorage.getItem("mlsuite.activeOrganizationSlug");
  return value && value.trim() ? value.trim() : null;
};

const mergeHeaders = (headers?: HeadersInit): Headers => {
  const merged = new Headers(headers);
  const tenantSlug = tenantHeader();
  if (tenantSlug) {
    merged.set("X-Organization-Slug", tenantSlug);
  }
  return merged;
};
...
const isTenantContractError = (dto: ErrorDto) =>
  (dto.status === 400 && dto.message.includes("tenant header")) ||
  (dto.status === 403 && dto.message.includes("organization"));
