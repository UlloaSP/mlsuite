  ...
  return (
    <div className="flex size-full flex-col overflow-hidden bg-[var(--sidebar-bg)] backdrop-blur-xl">
      <div className="flex flex-1 flex-col overflow-hidden">
        <nav aria-label="Main navigation" className="app-scroll flex-1 overflow-y-auto">
          <SidebarSection collapsed={collapsed} className="p-4">
            {navigation.map((item) => (
              <div key={item.to} className="space-y-1">
                <Link to={item.to} className={cx(FOCUS_RING, "flex rounded-xl")}>
                  <SidebarTile
                    icon={item.icon}
                    label={item.label}
                    isActive={
                ...
                {!collapsed && item.children && location.pathname.startsWith(item.to) && (
