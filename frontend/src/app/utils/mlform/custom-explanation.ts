...
import { customExplanationTemplate } from "./custom-explanation-template";
import {
  type CustomExplanationResult,
  type NormalizedCustomExplanationResult,
  normalizeCustomExplanationResult,
} from "./custom-explanation-result";
...
const toCatalogDefinition = async (
  item: PluginDto,
): Promise<CatalogExplanationDefinition | null> => {
  ...
  return {
    ...
    active: item.active,
    kind: detected.kind,
    definition: (await resolveCustomExplanationDefinitionWithFeedback(
      item.source,
    )) as ExplanationDefinition<ExplanationConfig> & ExplanationDefinitionWithFeedback,
  };
};
