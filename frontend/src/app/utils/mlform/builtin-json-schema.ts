...
import { BUILTIN_FIELD_DEFINITIONS, BUILTIN_FIELD_KINDS, BUILTIN_REPORT_DEFINITIONS, BUILTIN_REPORT_KINDS } from "./builtin-registry";
...
const toLiteralConst = (schema: { def?: { values?: readonly unknown[] } }): JsonSchema =>
	schema.def?.values?.length === 1 ? { const: schema.def.values[0] } : { enum: schema.def?.values ?? [] };
