...
const appendReportIssues = (
  ...
  schema.reports.forEach((report, index) => {
    if (
      ...
      BUILTIN_REPORT_KINDS.has(report.kind)
    ) {
      return;
    }
...
const mergeIssues = (
  ...
  const filteredIssues = baseIssues.filter((issue) => {
    ...
    if (section === "fields" && Array.isArray(schema.fields)) {
      ...
      return (
        !isRecord(field) || typeof field.kind !== "string" || BUILTIN_FIELD_KINDS.has(field.kind)
      );
    }
  ...
  const allFieldKinds = new Set(
    (options.customFieldDefinitions ?? []).map((definition) => definition.kind),
  );
  ...
  const allReportKinds = new Set(
    (options.customReportDefinitions ?? []).map((definition) => definition.kind),
  );
