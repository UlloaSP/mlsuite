...
const SHARED_REPORT_KEYS = ["id", "label", "description", "source", "ui"] as const;
...
export const filterInactiveCustomDefinitionsFromSchema = (
  ...
  return {
    ...
    reports: (schema.reports ?? []).filter(
      (report) =>
        engineRegistry.getReport(report.kind) !== undefined || activeReportKinds.has(report.kind),
    ),
    explanations: (schema.explanations ?? []).filter(
      (explanation) =>
        engineRegistry.getExplanation(explanation.kind) !== undefined ||
        activeKinds.has(explanation.kind),
    ),
  };
};
