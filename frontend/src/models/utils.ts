...
import { toMlformSchema } from "../app/utils/mlform/index";
...
export const toIdString = (value: unknown): string => toDisplayString(value);
...
export const getPredictionStatus = (status: unknown): PredictionFeedbackStatus => {
  const normalized = toDisplayString(status || "PENDING").toUpperCase();
