...
export function ModelsPage() {
	...
	const visibleItems = enrichedModels
		.filter(({ model }) => {
			...
			return [model.name, model.type, model.specificType, model.fileName]
				.some((value) => value.toLowerCase().includes(normalizedQuery));
		})
		.sort((left, right) => {
			switch (sort) {
				case "name":
					return left.model.name.localeCompare(right.model.name, undefined, { sensitivity: "base" });
				case "algorithm":
				...
				case "updated":
				default:
					return new Date(right.model.createdAt).getTime() - new Date(left.model.createdAt).getTime();
			}
		});
