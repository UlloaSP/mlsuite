...
const CREATE_PREDICTION_SUBHEADER = "Submit a new prediction for your model's signature"
...
export function CreatePredictionHeader({ isEditorActive, onToggleMode }: CreatePredictionHeaderProps) {
    const navigate = useNavigate()
    const [schemaErrors,] = useAtom(schemaErrorsAtom)
    const hasErrors = schemaErrors.length > 0
    return (
        <motion.div className={`flex flex-row max-h-fit justify-between ${!isEditorActive ? "px-4 pt-4" : ""}`}>
            <motion.div className="flex flex-col max-h-fit">
                <motion.div
                    className="flex-start justify-self-start flex flex-col">
                    <motion.button
