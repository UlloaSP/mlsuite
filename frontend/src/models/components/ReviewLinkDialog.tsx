...
import { useCreateReviewLinkMutation, useReviewLinks, useRevokeReviewLinkMutation } from "../../review/hooks";
				...
				<div className="grid min-h-0 flex-1 lg:grid-cols-[minmax(0,1fr)_360px]">
					<section className="min-h-0 border-r border-[var(--border-soft)] px-6 py-5">
						<div className="mb-4 flex flex-wrap items-center justify-between gap-3">
							...
							<div className="flex gap-2">
								<AppButton type="button" variant="secondary" className="rounded-md px-3 py-2" onClick={() => setSelectedIds(new Set(predictions.map((item) => item.id)))}>
									Select all
								</AppButton>
								<AppButton type="button" variant="ghost" className="rounded-md px-3 py-2" onClick={() => setSelectedIds(new Set())}>
									Deselect all
								</AppButton>
							</div>
						</div>
						<div className="max-h-[58vh] overflow-auto border-y border-[var(--border-soft)]">
							{predictions.map((prediction) => {
								...
								return (
									...
									>
										<span className={`grid size-6 place-items-center rounded-md border text-sm font-semibold ${selected ? "border-[var(--accent-primary)] bg-[var(--accent-primary)] text-white" : "border-[var(--border-strong)] text-transparent"}`}>
											✓
										</span>
										<span className="min-w-0 truncate text-sm font-semibold">{prediction.name}</span>
										<span className="text-sm text-[var(--text-secondary)]">{formatTimestamp(getPredictionTimestamp(prediction))}</span>
									</button>
								);
							})}
						</div>
					...
					<aside className="min-h-0 space-y-5 overflow-auto px-6 py-5">
						<div className="flex gap-3">
							<AppTextField type="date" value={expiresAt} onChange={(event) => setExpiresAt(event.currentTarget.value)} className="min-w-0 flex-1 rounded-md" />
							<AppButton type="button" disabled={selectedPredictions.length === 0 || createMutation.isPending} onClick={() => void createLink()} className="rounded-md px-4">
								<Link2 size={16} />
								Generate
							</AppButton>
						</div>
						<div>
							<p className="mb-3 text-xs font-semibold uppercase tracking-[0.16em] text-[var(--text-muted)]">Links</p>
								...
								{activeLinks.map((link) => {
									...
									return (
										<div key={link.id} className="space-y-2 py-4">
											...
											<p className="text-xs text-[var(--text-secondary)]">Created {formatTimestamp(link.createdAt)} · Expires {formatTimestamp(link.expiresAt)}</p>
											<div className="flex gap-2">
												<AppButton type="button" variant="secondary" disabled={!url} onClick={() => void copy(url)} className="rounded-md px-3 py-2">
													<Copy size={15} />
													Copy
												</AppButton>
												<AppButton type="button" variant="danger" disabled={Boolean(link.revokedAt) || revokeMutation.isPending} onClick={() => revokeMutation.mutate(link.id)} className="rounded-md px-3 py-2">
