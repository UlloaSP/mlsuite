...
import type { ExportPredictionReviewSummary, ExportReviewSelection } from "./export-review-selection";
...
export function ExportReviewPredictionRow({
	...
	const selectedReviews = summary.reviewers.filter((item) =>
		isReviewSelected(selection, predictionId, item.reviewer)).length;
		...
		<section className="border-b border-[var(--border-soft)] last:border-b-0">
			<div className="grid grid-cols-[auto_minmax(0,1fr)_auto_auto] items-center gap-3 px-4 py-3">
				...
				<button type="button" onClick={onToggleOpen} className="min-w-0 text-left">
					<p className="truncate text-sm font-semibold text-[var(--text-primary)]">{summary.prediction.name}</p>
					<p className="text-xs text-[var(--text-secondary)]">{formatTimestamp(getPredictionTimestamp(summary.prediction))}</p>
				</button>
			...
			{open ? (
				<div className="border-t border-[var(--border-soft)] px-4 py-2">
					{summary.reviewers.length === 0 ? (
						...
						return (
							<div key={key} className="border-b border-[var(--border-soft)] py-3 last:border-b-0">
								<div className="flex items-center justify-between gap-3">
									...
									<p className="text-xs text-[var(--text-secondary)]">
										{reviewer.outputFeedback.length} output · {reviewer.explanationFeedback.length} explanation
									</p>
								</div>
								<div className="mt-2 space-y-1 pl-8 text-xs text-[var(--text-secondary)]">
									...
									{reviewer.explanationFeedback.map((item) => (
										<p key={`e-${item.id}`}>Explanation {item.order + 1}: {valueText(item.realValue ?? item.value)}</p>
									))}
								</div>
							</div>
						);
