import { CheckCircle, ChevronDown, ChevronUp, Edit3, Save, X, XCircle } from "lucide-react"
...
export function PredictionDetailPanel({
    ...
    const [feedbackState, setFeedbackState] = useState<"COMPLETED" | "FAILED" | "PENDING">("PENDING")
    ...
    const { data: targs = [] } = useGetTargets({ predictionId: prediction.id || "" });
    ...
    useEffect(() => {
        setStatus(prediction.status.toString() as "PENDING" | "COMPLETED" | "FAILED");
        let outputs: any[] = [];
        // @ts-ignore
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-6">
                            ...
                            <div className="space-y-4">
                                <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4">
                                    <h3 className="font-semibold text-gray-900 dark:text-white mb-3">{prediction.name}</h3>
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex items-center justify-between">
                                            ...
                                            <span className="font-mono text-sm text-gray-900 dark:text-white">
                                                {/* @ts-ignore */}
                                                {formatExecutionTime(prediction.prediction.outputs[0].execution_time)}
                                            </span>
                                        </div>
                                        <div className="flex items-center justify-between">
    ...
    hover:after:opacity-100"
                                onClick={() => { navigate(`/models/${prediction.modelId}/signatures/${prediction.signatureId}/predictions/create/${encodeURIComponent(JSON.stringify(prediction.inputs))}`) }}
                            >
                            ...
                            <ExportButton name={prediction.name} inputs={prediction.inputs} targs={targs} />
                            ...
                            <div className="space-y-4">
                                ...
                                {status === "PENDING" || isEditing ? (
                                    <div className="space-y-4">
                                        <div className="text-sm text-gray-600 dark:text-gray-400 mb-3">Was this prediction correct?</div>
                                                    ...
                                                    {targs.map((target: TargetDto) => (
                                                        <div key={target.id} className="space-y-3">
                                                            ...
                                                            <input
                                                                ...
                                                                onChange={(e) => setActualValues(prev => ({
                                                                    ...prev,
                                    ...
                                    <div className="flex flex-col gap-2 bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
                                        ...
                                        {status === "FAILED" && (
                                            <div className="space-y-2">
                                                <button
                                                    onClick={() => setCorrectValuesOpen(!correctValuesOpen)}
