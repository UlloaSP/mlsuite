...
export function ExportReviewModal({
  ...
  const togglePrediction = (predictionId: string) =>
    update((draft) => {
      if (draft.excludedPredictionIds.has(predictionId))
        draft.excludedPredictionIds.delete(predictionId);
  ...
  const togglePredictionReviewer = (predictionId: string, reviewer: string) =>
    update((draft) => {
        ...
        <header className="flex items-start justify-between gap-4 border-b border-[var(--border-soft)] px-6 py-5">
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--text-secondary)]">
              {selectedPredictions}/{predictions.length} predictions · {selectedReviewerCount}/
            ...
            <div className="mb-3 flex items-center justify-between gap-3">
              ...
              <div className="flex gap-2">
                <AppButton
                  ...
                  onClick={() => setSelection(emptyExportReviewSelection())}
                >
                  Select all
                </AppButton>
                <AppButton
                  ...
                  onClick={() =>
                    update((draft) =>
                      predictions.forEach((item) => draft.excludedPredictionIds.add(item.id)),
                    )
                  }
                >
                  Deselect all
