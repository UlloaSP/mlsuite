			...
			<AppPanel className="space-y-4">
				...
				<div className="grid gap-3 sm:grid-cols-3">
					<div>
						...
						<p className="mt-1 text-lg font-semibold text-[var(--text-primary)]">{stats.fieldCount}</p>
					</div>
					<div>
						<p className="text-xs uppercase tracking-[0.18em] text-[var(--text-muted)]">Reports</p>
						<p className="mt-1 text-lg font-semibold text-[var(--text-primary)]">{stats.reportCount}</p>
					</div>
					<div>
						<p className="text-xs uppercase tracking-[0.18em] text-[var(--text-muted)]">Explanations</p>
						<p className="mt-1 text-lg font-semibold text-[var(--text-primary)]">{stats.explanationsEnabled ? "Enabled" : "Disabled"}</p>
					</div>
				</div>
			</AppPanel>
