...
export function CreateSignatureActionSection() {
  ...
  const isFormValid = schemaErrors <= 0 && signatureName.trim() && version && signatureId;
  ...
  const handleSaveSignature = async () => {
    ...
    try {
      ...
      void navigate(`/models/${modelId}?tab=signatures`);
    } catch {
    } finally {
      setIsLoading(false);
    }
    ...
    <motion.div className="shrink-0">
      <div className="grid gap-4 xl:grid-cols-[minmax(240px,1.2fr)_minmax(280px,1.4fr)_minmax(360px,1.7fr)_auto]">
        <div className="space-y-2">
          <label className="text-sm font-semibold text-[var(--text-primary)]">Base Version</label>
          <AppSelect
            value={signatureId}
          ...
          >
            {signatures.length ? null : <option value="">No base signatures available</option>}
        ...
        <div className="space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <label className="text-sm font-semibold text-[var(--text-primary)]">Version Bump</label>
            {selectedSignature ? (
