		...
		<div className="space-y-8">
			<AppPanel className="space-y-3">
				<div className="flex items-center justify-between">
					...
					<span className="font-mono font-medium text-[var(--text-primary)]">{outputType || "N/A"}</span>
				</div>
				<div className="flex items-center justify-between">
					<span className="text-sm font-semibold text-[var(--text-secondary)]">Execution Time:</span>
					<span className="font-mono font-medium text-[var(--text-primary)]">{executionTime || "N/A"}</span>
				</div>
			</AppPanel>
			...
			<AppPanel className="space-y-4">
				...
				<div className="space-y-3">
					{Object.entries(inputs).map(([key, value]) => (
						<div key={key} className="flex items-center justify-between rounded-[20px] bg-[var(--surface-muted)] p-3">
							<span className="text-sm font-medium text-[var(--text-secondary)]">{key}:</span>
							<span className="font-mono text-sm text-[var(--text-primary)]">{formatInputValue(value)}</span>
						</div>
					))}
				</div>
			</AppPanel>
			...
			<AppPanel className="space-y-4">
				...
				<div className="space-y-3">
					{Object.entries(targets).map(([key, value]) => (
						<div key={key} className="flex items-center justify-between rounded-[20px] bg-[var(--surface-muted)] p-3">
							<span className="text-sm font-medium text-[var(--text-secondary)]">{getTargetLabel(signatureSchema, Number(key))}:</span>
			...
			<AppPanel className="space-y-4 p-6">
				...
				<div className="flex gap-4 pt-2">
					<AppButton onClick={onCancel} variant="secondary" className="flex-1">Cancel</AppButton>
