...
export function CreatePredictionModal({ prediction, inputs }: CreatePredictionModalProps) {
    ...
    useEffect(() => {
        ...
        let targets: Record<number, number | string> = {};
                ...
                >
                    ...
                    >
                        <motion.div
                            ...
                            <motion.h1
                                className="text-5xl leading-20 font-bold bg-gradient-to-r from-gray-900 to-violet-600 dark:from-white dark:to-violet-400 bg-clip-text text-transparent"
                            >
                        ...
                        <motion.div className="flex flex-row flex-1 overflow-hidden">
                            ...
                            <motion.div className="flex flex-col flex-1 p-8 gap-8 overflow-hidden">
                                ...
                                <motion.div className="min-h-fit bg-gray-50 dark:bg-gray-800 rounded-lg p-3">
                                    <motion.div className="flex items-center justify-between ">
                                        <motion.span className="text-sm font-semibold text-gray-600 dark:text-gray-400">Type:</motion.span>
                                        <motion.span className={`font-mono font-medium text-gray-800 dark:text-gray-200`}>
                                            {/* @ts-ignore */}
                                            {prediction.outputs[0]?.type || "N/A"}
                                        </motion.span>
                                    </motion.div>
                                    <motion.div className="flex items-center justify-between">
                                        <motion.span className="text-sm font-semibold text-gray-600 dark:text-gray-400">Execution Time:</motion.span>
                                        <motion.span className={`font-mono font-medium text-gray-800 dark:text-gray-200`}>
                                            {/* @ts-ignore */}
                                    ...
                                    <motion.div className="flex items-center justify-between">
                                        <motion.span className="text-sm font-semibold text-gray-600 dark:text-gray-400">Status:</motion.span>
                                        <motion.span className={`font-mono font-medium text-gray-800 dark:text-gray-200`}>
                                            {"pending"}
                                        </motion.span>
                                    </motion.div>
                                    ...
                                    <div className="space-y-3 overflow-y-auto">
                                        {Object.entries(inputs).map(([key, value]) => (
                                            <div key={key} className="flex items-center justify-between bg-gray-50 dark:bg-gray-800 rounded-lg p-3">
                                                ...
                                                <span className="font-mono text-sm text-gray-900 dark:text-white">{formatInputValue(value)}</span>
                                            </div>
                                        ))}
                                    </div>
                                </motion.div>
                            ...
                            <motion.div className="flex flex-col flex-1 gap-8 p-8 overflow-hidden">
                                {/* Targets */}
                                <motion.div className="flex flex-col flex-1 space-y-4 overflow-hidden bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
                                    <h3 className="font-semibold text-gray-900 dark:text-white">Targets</h3>
                                    <motion.div className="space-y-3 overflow-y-auto">
                                        {Object.entries(targets).map(([key, value]) => (
                                            <motion.div key={key} className="flex items-center justify-between bg-gray-50 dark:bg-gray-800 rounded-lg p-3">
                                                <motion.span className="text-sm font-medium text-gray-700 dark:text-gray-300">target_{key}:</motion.span>
                                                <motion.span className="font-mono text-sm text-gray-900 dark:text-white">{value.toString()}</motion.span>
                                            </motion.div>
                                        ))}
                                    </motion.div>
                                </motion.div>
