...
import { csvEscape, flattenRecord, getExplanationHeaders, toCsvCell } from "../export-csv";
...
export function ExportButton({ predictions, delimiter = ",", signatureSchema }: ExportButtonProps) {
  ...
  const { headers, rows } = useMemo(() => {
    ...
    const inputKeys = Object.keys(flattenRecord(predictions[0].inputs)).sort();
    ...
    const nextRows = predictions.flatMap((prediction, index) => {
      ...
      const inputs = flattenRecord(prediction.inputs);
      const targetMap = new Map(targets.map((target) => [target.order, target]));
      const outputFeedbackByOrder = new Map(
        ((outputFeedbackQueries[index]?.data ?? []) as OutputFeedbackDto[]).map((item) => [
          item.order,
          item,
        ]),
      );
      ...
      const feedbackByOrder = new Map(
        ((explanationFeedbackQueries[index]?.data ?? []) as modelApi.ExplanationFeedbackDto[]).map(
          (item) => [item.order, item],
        ),
      );
      const explanationValues = explanationEntries.flatMap((explanation) => {
        const cells = [toCsvCell(explanation.content.join("\n\n"))];
        ...
        return [
          ...
          ...getQuestionnaireFieldIds(explanation.feedbackQuestionnaire).map((fieldId) =>
            toCsvCell(feedbackValues[fieldId]),
          ),
        ];
      });

      return [
        [...inputKeys.map((key) => toCsvCell(inputs[key])), ...targetValues, ...explanationValues],
      ];
    });
