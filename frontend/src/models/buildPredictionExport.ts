...
import type { ExplanationFeedbackDto, OutputFeedbackDto, PredictionDto, TargetDto } from "./api/modelService";
...
export function buildPredictionExportData({
	...
	const reviewerLabels = collectReviewerLabels(outputFeedbackByPrediction, explanationFeedbackByPrediction);
		...
		const targetValues = (schema?.reports ?? []).flatMap((_report: ReportConfig, order: number) => {
			...
			const feedbackValues = reviewerLabels.map((reviewer) => {
				...
				const userRealValue = assessmentRaw !== ""
					...
					: "";
				return toCell(userRealValue !== ""
					? getSchemaAwareTargetValue(userRealValue, signatureSchema, order, prediction.prediction)
					: "");
			});
			return [toCell(predicted), ...feedbackValues];
		});

		const explanationEntries = explanationHeaders.length > 0
