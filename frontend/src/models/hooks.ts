...
export const useGetExplanationFeedback = ({
  predictionId,
}: modelApi.GetExplanationFeedbackRequest) =>
  useQuery({
...
export function useCreateModelMutation() {
  ...
  return useMutation({
    ...
    onSuccess: (created: modelApi.CreateModelDto) => {
      ...
      void qc.invalidateQueries({ queryKey: ["getSignatures"] });
    },
  });
}
...
export function useCreatePredictionMutation() {
  ...
  return useMutation({
    ...
    onSuccess: (prediction: modelApi.PredictionDto) => {
      ...
      void qc.invalidateQueries({ queryKey: ["getPredictions"] });
    },
  });
}
...
export function useCreateTargetMutation() {
  ...
  return useMutation({
    ...
    onSuccess: (target: modelApi.TargetDto) => {
      ...
      void qc.invalidateQueries({ queryKey: ["getTargets"] });
    },
  });
}
...
export function useCreateExplanationFeedbackMutation() {
  ...
  return useMutation({
    ...
    onSuccess: (explanationFeedback: ExplanationFeedbackDto) => {
      ...
      void qc.invalidateQueries({ queryKey: ["getExplanationFeedback"] });
    },
  });
}
  ...
  return useMutation({
    ...
    onSuccess: (prediction: PredictionDto) => {
      qc.setQueryData<PredictionDto[]>(
        GET_PREDICTIONS_QUERY_KEY({ signatureId: prediction.signatureId }),
        (prev) =>
          prev ? prev.map((p) => (p.id === prediction.id ? prediction : p)) : [prediction],
      );
      void qc.invalidateQueries({ queryKey: ["getPredictions"] });
      void qc.invalidateQueries({ queryKey: ["getTargets"] });
    },
  ...
  return useMutation({
    ...
    onSuccess: (explanationFeedback: ExplanationFeedbackDto) => {
      qc.setQueryData<ExplanationFeedbackDto[]>(
        ...
        (prev) =>
          prev
            ? prev.map((item) => (item.id === explanationFeedback.id ? explanationFeedback : item))
            : [explanationFeedback],
      );
      void qc.invalidateQueries({ queryKey: ["getExplanationFeedback"] });
      void qc.invalidateQueries({ queryKey: ["getPredictions"] });
