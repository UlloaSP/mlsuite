...
export function useCreateModelMutation() {
	...
	return useMutation({
		...
		mutationFn: (data: modelApi.CreateModelRequest) =>
			modelApi.createModel(data),
		onSuccess: (CreateModel: modelApi.CreateModelDto) => {
			queryClient.setQueryData<ModelDto[]>(GET_MODELS_QUERY_KEY, (prev) =>
				prev ? [CreateModel.model, ...prev] : [CreateModel.model],
			);
			queryClient.setQueryData<SignatureDto[]>(
				...
				(prev) => {
					...
					if (!prev && CreateModel.signatureFromDataframe) {
						return [
							CreateModel.signatureFromDataframe,
							CreateModel.signatureFromModel,
						];
					}
...
export function useCreateSignatureMutation() {
	...
	return useMutation({
		...
		mutationFn: (data: modelApi.CreateSignatureRequest) =>
			modelApi.createSignature(data),
...
export function useCreatePredictionMutation() {
	...
	return useMutation({
		...
		mutationFn: (data: modelApi.CreatePredictionRequest) =>
			modelApi.createPrediction(data),
...
export function useCreateTargetMutation() {
	...
	return useMutation({
		...
		mutationFn: (data: modelApi.CreateTargetRequest) =>
			modelApi.createTarget(data),
...
export function useUpdatePredictionMutation() {
	...
	return useMutation({
		...
		mutationFn: (data: modelApi.UpdatePredictionRequest) =>
			modelApi.updatePrediction(data),
...
export function useUpdateTargetMutation() {
	...
	return useMutation({
		...
		mutationFn: (data: modelApi.UpdateTargetRequest) =>
			modelApi.updateTarget(data),
		onSuccess: (target: modelApi.TargetDto) => {
			queryClient.setQueryData<modelApi.TargetDto[]>(
				GET_TARGETS_QUERY_KEY({ predictionId: target.predictionId }),
				(prev) =>
...
export const GET_SIGNATURES_QUERY_KEY = ({
	modelId,
}: modelApi.GetAllSignaturesRequest) => ["getSignatures", { modelId }] as const;

export const useGetSignatures = ({
	modelId,
}: modelApi.GetAllSignaturesRequest) =>
	useQuery({
...
export const GET_PREDICTIONS_QUERY_KEY = ({
	signatureId,
}: modelApi.GetPredictionsRequest) =>
	["getPredictions", { signatureId }] as const;
...
export const GET_TARGETS_QUERY_KEY = ({
	predictionId,
}: modelApi.GetTargetsRequest) => ["getTargets", { predictionId }] as const;
...
export const GET_SIGNATURE_QUERY_KEY = ({
	signatureId,
}: modelApi.GetSignatureRequest) => ["getSignature", { signatureId }] as const;

export const useGetSignature = ({
	signatureId,
}: modelApi.GetSignatureRequest) =>
	useQuery({
