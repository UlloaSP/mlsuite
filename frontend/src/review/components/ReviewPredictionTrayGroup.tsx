...
export function ReviewPredictionTrayGroup({
	...
	const countTone = tone === "revision" ? "bg-[var(--success-quiet)] text-[var(--success-text)]" : "bg-[var(--warning-quiet)] text-[var(--warning-text)]";

	return (
		<section ref={sectionRef} className="shrink-0 border-b border-[var(--border-soft)] pb-4 last:border-b-0">
			<div ref={headerRef} className="flex shrink-0 items-center gap-3">
				...
				<span className={`rounded-md px-2.5 py-1.5 text-xs font-semibold ${countTone}`}>{count}</span>
				<button
					type="button"
					onClick={onToggle}
					aria-expanded={open}
			...
			{open && count > 0 ? (
				<div ref={listRef} className="mt-3 overflow-y-auto" style={{ maxHeight: `${listHeight}px` }}>{children}</div>
			) : null}
		</section>
	);
}
