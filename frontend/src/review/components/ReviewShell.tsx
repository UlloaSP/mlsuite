...
const nextTheme = (mode: ThemeMode): ThemeMode => mode === "system" ? "light" : mode === "light" ? "dark" : "system";
const themeLabel = (mode: ThemeMode) => mode === "system" ? "System" : mode === "light" ? "Light" : "Dark";
const themeIcon = (mode: ThemeMode) => mode === "system" ? Laptop : mode === "light" ? Sun : Moon;

export function ReviewShell({ title = "External Review", showLogout = true, children }: ReviewShellProps) {
				...
				<div className="mx-auto flex max-w-6xl items-center justify-between gap-4">
					...
					{showLogout ? (
						<div className="flex items-center gap-2">
							<AppButton type="button" variant="ghost" onClick={() => setThemeMode(nextTheme(themeMode))}>
								<ThemeIcon size={16} />
