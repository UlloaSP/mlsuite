...
import type { ExplanationFeedbackDto, OutputFeedbackDto, TargetDto } from "../../models/api/modelService";
...
export function ReviewCombinedFeedbackForm(props: ReviewCombinedFeedbackFormProps) {
	...
	const steps = useMemo(() => buildReviewFeedbackSteps({
		targets,
		outputFeedbackByOrder,
		explanationFeedbackByOrder,
		reports,
		...
		predictionValue,
		explanations,
	}), [explanationFeedbackByOrder, explanations, outputFeedbackByOrder, predictionValue, reports, signatureSchema, targets]);
						...
						<div key={step.id} className="p-4">
							...
							{buildQuestionnaireFormSchema(step.schema).fields.map((field: FieldConfig) => (
								<p key={String(field.id)} className="mt-2 text-sm text-[var(--text-secondary)]">
									<span className="font-medium text-[var(--text-primary)]">{String(field.label ?? field.id)}:</span>{" "}
