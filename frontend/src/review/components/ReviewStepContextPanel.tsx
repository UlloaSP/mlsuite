	...
	return (
		<aside className="lg:sticky lg:top-28">
			<div className="border border-[var(--border-soft)] bg-[var(--surface-primary)] p-4 shadow-[var(--shadow-card)]">
				<p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[var(--accent-primary)]">{title}</p>
				<h3 className="mt-2 text-base font-semibold leading-5 text-[var(--text-primary)]">{step.title}</h3>
				<div className="mt-4 space-y-2 text-sm leading-6 text-[var(--text-secondary)]">
					{content.length > 0 ? (
						content.map((item) => (
							<p key={`${step.id}-${item.key}`} className="break-words">{item.text}</p>
						))
					) : (
