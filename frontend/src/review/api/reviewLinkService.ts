...
export const createReviewExplanationFeedback = (
  token: string,
  request: CreateExplanationFeedbackRequest,
) =>
...
export const updateReviewExplanationFeedback = (
  token: string,
  request: UpdateExplanationFeedbackRequest,
) =>
  appFetch<ExplanationFeedbackDto>(
    `/api/review-links/token/${encodeURIComponent(token)}/explanation-feedback`,
    json("PATCH", request),
