...
export function ProfilePage() {
    ...
    return (
        <motion.div className="flex flex-1 size-full overflow-hidden bg-gradient-to-br from-slate-50 via-yellow-50 to-amber-50 dark:from-gray-900 dark:via-slate-800 dark:to-amber-900">
            ...
            >
                <ProfileHeader imageUrl={user?.avatarUrl} name={user?.userName || user?.fullName || "Guest"} provider={user ? `Logged in with ${user.oauthProvider.charAt(0).toUpperCase() + user.oauthProvider.slice(1)}` : "Not logged in"} />
