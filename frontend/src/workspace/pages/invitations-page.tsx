...
export function InvitationsPage() {
	...
	const { data: roles } = useQuery({ queryKey: ["roles", id], queryFn: () => getRoles(id), enabled: Boolean(id && workspace?.permissions.canManageInvitations) });
	...
	const roleOptions = invitationRoleOptions(roles?.roles ?? [], workspace?.permissions.canTransferOwnership ?? false);
							...
							<tbody>
								{filtered.map((invite) => (
									<tr key={invite.id} className="border-b border-[var(--border-soft)] last:border-0">
										...
										<td><RoleBadge value={invite.roleDefinition?.name ?? invite.role} /></td>
