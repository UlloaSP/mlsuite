...
export function ThemeSelectionCard({
  ...
  return (
    <article className="relative flex flex-col rounded-2xl border border-line bg-surface-subtle p-3 transition hover:border-line-strong">
      <ThemePresetPreview
        label={label}
        preview={preview}
      ...
      <button
        ...
        className="absolute inset-0 rounded-2xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 focus-visible:ring-offset-surface"
        onClick={() => onApply("both")}
      />
      <span className="pointer-events-none relative mt-auto min-w-0 truncate px-1 pt-4 text-sm font-semibold text-fg">
        {label}
      </span>
    </article>
  );
